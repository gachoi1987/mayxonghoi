...
function siy_get_user_orders($user_id, $number = 10, $start = 0) {
	...
	while ($row = $GLOBALS['db']->fetchRow($res)) {
		if ($row['order_status'] == OS_UNCONFIRMED) {
			$row['handler'] = "<a href=\"user.php?act=cancel_order&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_cancel']."')) return false;\" class=\"button dim_button\"><span>".$GLOBALS['_LANG']['cancel']."</span></a>";
		} else if ($row['order_status'] == OS_SPLITED) {
			if ($row['shipping_status'] == SS_SHIPPED) {
				@$row['handler'] = "<a href=\"user.php?act=affirm_received&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_received']."')) return false;\" class=\"button\"><span>".$GLOBALS['_LANG']['received']."</span></a>";
			} elseif ($row['shipping_status'] == SS_RECEIVED) {
				@$row['handler'] = '<span class="status shipping_status_2">'.$GLOBALS['_LANG']['ss_received'] .'</span>';
			} else {
				if ($row['pay_status'] == PS_UNPAYED) {
					@$row['handler'] = "<a href=\"user.php?act=order_detail&order_id=" .$row['order_id']. '" class="button"><span>' .$GLOBALS['_LANG']['pay_money']. '</span></a>';
				} else {
					@$row['handler'] = "<a href=\"user.php?act=order_detail&order_id=" .$row['order_id']. '" class="button dim_button"><span>' .$GLOBALS['_LANG']['view_order']. '</span></a>';
				}
			}
		} else {
