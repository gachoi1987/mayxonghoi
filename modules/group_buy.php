...
if (empty($param))
{
    ...
    $act = $param[0] == 'dat-hang' ? 'buy' : 'detail';
}
...
if ($act == 'list')
{
    ...
    if (!$smarty->is_cached('group_buy_list.dwt', $cache_id))
    {
        if ($count > 0)
        {
            ...
            /* 设置分页链接 */
            $pager = get_pager('/nhom-mua', array('act' => 'list'), $count, $page, $size);
            $smarty->assign('pager', $pager);
        }
        /* 模板赋值 */
        $smarty->assign('cfg', $_CFG);
        assign_template();
        $position = assign_ur_here('', '', 'group_buy');
...
{
    //edit buy nobj
    $group_buy_id = $param[1] != '' ? intval($param[1]) : 0;
