    ...
    else
    {
        ...
        if (!empty($_POST['act']) && $_POST['act'] == 'signin')
        {
            ...
            if ($user->login($_POST['username'], $_POST['password'],isset($_POST['remember'])))
            {
                ...
                if ($db->getOne($sql) > 0)
                {
                    ecs_header("Location: gio-hang?step=checkout\n");
                }
                else
                {
                    ecs_header("Location: ");
            ...
            {
                $_SESSION['login_fail']++;
                show_message($_LANG['signin_failed'], '', '/gio-hang?step=login');
            }
        }
        elseif (!empty($_POST['act']) && $_POST['act'] == 'signup')
        {
            ...
            {
                /* 用户注册成功 */
                ecs_header("Location: gio-hang?step=consignee\n");
                exit;
            }
            else
            {
...
elseif ($_REQUEST['step'] == 'consignee')
{
    ...
    else
    {
        ...
        $consignee = array(
            ...
            'mobile'        => empty($_POST['mobile'])     ? '' :   compile_str(trim($_POST['mobile'])),
...
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    if (empty($_SESSION['direct_shopping']) && $_SESSION['user_id'] == 0)
    {
        ...
        ecs_header("Location: gio-hang?step=login\n");
        exit;
    }
...
elseif ($_REQUEST['step'] == 'update_cart')
{
    ...
    // show_message($_LANG['update_cart_notice'], $_LANG['back_to_cart'], 'gio-hang');
    // exit;
    ecs_header("Location: gio-hang\n");
    exit;
}
...
else
{
    ...
    /** Start để điền consignee ngay tai gio hang*/
        ...
        /* 返回收货人页面代码 */
        $smarty->assign('real_goods_count', exist_real_goods(0, $_SESSION['flow_type']) ? 1 : 0);
    /** End */
...
$smarty->display('flow'.$_device.'.dwt');
