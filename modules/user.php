...
$action = filter_var(strip_tags($action), FILTER_SANITIZE_STRING);
$title = isset($_LANG[$action]) ? $_LANG[$action] : $_LANG['user_center'];
$position = assign_ur_here(0, $title,'user');
...
if (empty($_SESSION['user_id']))
{
    if (!in_array($action, $not_login_arr))
    {
        if (in_array($action, $ui_arr))
        {
            ...
            {
                $back_act = $ecs->url().'thanh-vien?' . strip_tags($_SERVER['QUERY_STRING']);
            }
            $action = 'dang-nhap';
        }
        else
        {
            ecs_header("Location: {$ecs->url()}thanh-vien\n");
            exit;
        }
    }
}
...
if ($action == 'default')
{
    ...
    $_template = 'user_clips'.$_device.'.dwt';
    $smarty->display($_template);
}
...
if ($action == 'register')
{
    ...
    {
        $back_act = strpos($GLOBALS['_SERVER']['HTTP_REFERER'], $ecs->url().'thanh-vien') ? './' : $GLOBALS['_SERVER']['HTTP_REFERER'];
    }
...
//    $smarty->assign('back_act', $back_act);
    $_template = 'user_passport'.$_device.'.dwt';
    $smarty->display($_template);
}
...
elseif ($action == 'act_register')
{
    ...
    else
    {
        ...
        $confirm_password = isset($_POST['confirm_password']) ? trim($_POST['confirm_password']) : '';//david 2012-6-13
        ...
        if ((intval($_CFG['captcha']) & CAPTCHA_REGISTER) && gd_version() > 0)
        {
            ...
            {
                show_message($_LANG['invalid_captcha'], $_LANG['sign_up'], $ecs->url().'thanh-vien/?act=register', 'error');
            }
        ...
        if (register($username, $password, $email, $other) !== false)
        {
            ...
            show_message(sprintf($_LANG['register_success'], $username . $ucdata), array($_LANG['back_up_page'], $_LANG['profile_lnk']), array($back_act, $ecs->url().'thanh-vien'), 'success');
        }
        else
        {
            $err->show($_LANG['sign_up'], $ecs->url().'thanh-vien/?act=register');
        }
    }
}
...
elseif ($action == 'validate_email')
{
    ...
    if ($hash)
    {
        ...
        if ($id > 0)
        {
            ...
            $row = $db->getRow($sql);
            show_message(sprintf($_LANG['validate_ok'], $row['user_name'], $row['email']),$_LANG['profile_lnk'], $ecs->url().'thanh-vien');
        }
    }
...
elseif ($action == 'act_login')
{
    ...
    if (($captcha & CAPTCHA_LOGIN) && (!($captcha & CAPTCHA_LOGIN_FAIL) || (($captcha & CAPTCHA_LOGIN_FAIL) && $_SESSION['login_fail'] > 2)) && gd_version() > 0)
    {
        if (empty($_POST['captcha']))
        {
            show_message($_LANG['invalid_captcha'], $_LANG['relogin_lnk'], $ecs->url().'thanh-vien', 'error');
        }
    ...
    if ($user->login($username, $password,isset($_POST['remember'])))
    {
        ...
        //show_message($_LANG['login_success'] . $ucdata , array($_LANG['back_up_page'], $_LANG['profile_lnk']), array($back_act,$ecs->url().'thanh-vien'), 'success');
        ecs_header("Location: {$ecs->url()}thanh-vien\n");
        exit;
    }
    else
    {
        $_SESSION['login_fail'] ++ ;
        show_message($_LANG['login_failure'], $_LANG['relogin_lnk'], $ecs->url().'thanh-vien', 'error');
    }
}
...
elseif ($action == 'signin')
{
    ...
    if ($user->login($username, $password ,isset($_POST['remember'])))//david
    {
...
elseif ($action == 'logout')
{
    ...
    exit;
    //$ucdata = empty($user->ucdata)? "" : $user->ucdata;
    //show_message($_LANG['logout'] . $ucdata, array($_LANG['back_up_page'], $_LANG['back_home_lnk']), array($back_act, './'), 'info');
}
...
elseif ($action == 'profile')
{
    ...
    $_template = 'user_transaction'.$_device.'.dwt';
    $smarty->display($_template);
}
...
elseif ($action == 'act_edit_profile')
{
    ...
    {
        show_message($_LANG['edit_profile_success'], $_LANG['profile_lnk'], $ecs->url().'thanh-vien/?act=profile', 'success');
    }
    else
    {
...
elseif ($action == 'get_passwd_question')
{
    ...
    {
        show_message($_LANG['no_passwd_question'], $_LANG['user_center'], "thanh-vien", 'success');
    }
    else
    {
    ...
    {
        show_message($_LANG['no_passwd_question'], $_LANG['user_center'], "thanh-vien", 'notice');
    }
...
elseif ($action == 'check_answer')
{
    ...
    if (($captcha & CAPTCHA_LOGIN) && (!($captcha & CAPTCHA_LOGIN_FAIL) || (($captcha & CAPTCHA_LOGIN_FAIL) && $_SESSION['login_fail'] > 2)) && gd_version() > 0)
    {
        if (empty($_POST['captcha']))
        {
            show_message($_LANG['invalid_captcha'], $_LANG['back_retry_answer'], $ecs->url().'thanh-vien/?act=qpassword_name', 'error');
        }
    ...
    {
        show_message($_LANG['wrong_passwd_answer'], $_LANG['back_retry_answer'], $ecs->url().'thanh-vien/?act=qpassword_name', 'info');
    }
    else
    {
...
elseif ($action == 'send_pwd_email')
{
    ...
    if ($user_info && $user_info['email'] == $email)
    {
        ...
        {
            show_message($_LANG['send_success'] . $email, $_LANG['back_home_lnk'], './', 'success');
        }
        else
        {
            //发送邮件出错
            show_message($_LANG['fail_send_password'], $_LANG['back_page_up'], './', 'error');
        }
    }
    else
    {
        //用户名与邮件地址不匹配
        show_message($_LANG['username_no_email'], $_LANG['back_page_up'], '', 'error');
    }
}
...
elseif ($action == 'act_edit_password')
{
    ...
    if (($user_info && (!empty($code) && md5($user_info['user_id'] . $_CFG['hash_code'] . $user_info['reg_time']) == $code)) || ($_SESSION['user_id']>0 && $_SESSION['user_id'] == $user_id && $user->check_user($_SESSION['user_name'], $old_password)))
    {
        ...
        if ($user->edit_user(array('username'=> (empty($code) ? $_SESSION['user_name'] : $user_info['user_name']), 'old_password'=>$old_password, 'password'=>$new_password), empty($code) ? 0 : 1))
        {
			...
			$db->query($sql);
            $user->logout();
            show_message($_LANG['edit_password_success'], $_LANG['relogin_lnk'], $ecs->url().'thanh-vien/?act=login', 'success');
        }
        else
        {
...
elseif ($action == 'act_add_bonus')
{
    ...
    {
        show_message($_LANG['add_bonus_sucess'], $_LANG['back_up_page'], $ecs->url().'thanh-vien/?act=bonus', 'info');
    }
    else
    {
        $err->show($_LANG['back_up_page'], $ecs->url().'thanh-vien/?act=bonus');
    }
}
...
elseif ($action == 'order_list')
{
    ...
    $pager  = get_pager($ecs->url().'thanh-vien', array('act' => $action), $record_count, $page);
...
elseif ($action == 'cancel_order')
{
    ...
    {
        ecs_header("Location: {$ecs->url()}/thanh-vien/?act=order_list\n");
        exit;
    }
    else
    {
        $err->show($_LANG['order_list_lnk'], $ecs->url().'thanh-vien/?act=order_list');
    }
}
...
elseif ($action == 'act_edit_address')
{
    ...
    {
        show_message($_LANG['edit_address_success'], $_LANG['address_list_lnk'], $ecs->url().'thanh-vien/?act=address_list', 'success');
    }
}
...
elseif ($action == 'drop_consignee')
{
    ...
    {
        ecs_header("Location: {$ecs->url()}thanh-vien/?act=address_list\n");
        exit;
    }
    else
    {
...
elseif ($action == 'delete_collection')
{
    ...
    ecs_header("Location: {$ecs->url()}thanh-vien/?act=collection_list\n");
    exit;
}
...
elseif ($action == 'message_list')
{
    ...
    if ($order_id)
    {
        ...
        $order_info['url'] = $ecs->url().'thanh-vien/?act=order_detail&order_id=' . $order_id;
    }
    else
    {
        $sql = "SELECT COUNT(*) FROM " .$ecs->table('feedback').
    ...
    $pager = get_pager($ecs->url().'thanh-vien', $act, $record_count, $page, 5);
...
elseif ($action == 'comment_list')
{
    ...
    $record_count = $db->getOne($sql);
    $pager = get_pager($ecs->url().'thanh-vien', array('act' => $action), $record_count, $page, 5);
...
elseif ($action == 'act_add_message')
{
    ...
    {
        show_message($_LANG['add_message_success'], $_LANG['message_list_lnk'], $ecs->url().'thanh-vien/?act=message_list&order_id=' . $message['order_id'],'success');
    }
    else
    {
        $err->show($_LANG['message_list_lnk'], $ecs->url().'thanh-vien/?act=message_list');
    }
}
...
elseif ($action == 'act_del_tag')
{
    ...
    ecs_header("Location: {$ecs->url()}thanh-vien/?act=tag_list\n");
    exit;
...
elseif ($action == 'add_booking')
{
    ...
    $spec = isset($_GET['spec']) ? intval($_GET['spec']) : '';
    if ($goods_id == 0)
    {
...
elseif ($action == 'act_add_booking')
{
    ...
    {
        show_message($_LANG['booking_success'], $_LANG['back_booking_list'], $ecs->url().'thanh-vien/?act=booking_list','success');
    }
    else
    {
        $err->show($_LANG['booking_list_lnk'], $ecs->url().'thanh-vien/?act=booking_list');
    }
}
...
elseif ($action == 'act_del_booking')
{
    ...
    if ($id == 0 || $user_id == 0)
    {
        ecs_header("Location: {$ecs->url()}thanh-vien/?act=booking_list\n");
        exit;
    }
...
elseif ($action == 'act_account')
{
    ...
    if ($surplus['process_type'] == 1)
    {
        ...
        if ($surplus['rec_id'] > 0)
        {
            ...
            show_message($content, $_LANG['back_account_log'], $ecs->url().'thanh-vien/?act=account_log', 'info');
        }
        else
        {
...
elseif ($action == 'cancel')
{
    ...
    if ($id == 0 || $user_id == 0)
    {
        ecs_header("Location: {$ecs->url()}thanh-vien/?act=account_log\n");
        exit;
    }
...
elseif ($action == 'pay')
{
    ...
    if ($payment_id == 0)
    {
        ecs_header("Location: {$ecs->url()}thanh-vien/?act=account_deposit&id=".$surplus_id."\n");
        exit;
    }
...
elseif ($action == 'del_msg')
{
    ...
    ecs_header("Location: {$ecs->url()}thanh-vien/?act=message_list&order_id=$order_id\n");
    exit;
}
...
elseif ($action == 'del_cmt')
{
    ...
    ecs_header("Location: {$ecs->url()}thanh-vien/?act=comment_list\n");
    exit;
}
...
elseif ($action == 'merge_order')
{
    ...
    {
        show_message($_LANG['merge_order_success'],$_LANG['order_list_lnk'], $ecs->url().'thanh-vien/?act=order_list', 'success');
    }
    else
    {
...
elseif ($action == 'act_edit_surplus')
{
    ...
    if ($surplus <= 0)
    {
        ...
        $err->show($_LANG['order_detail'], $ecs->url().'thanh-vien/?act=order_detail&order_id=' . $order_id);
    }
    ...
    /* 跳转 */
    ecs_header('Location: {$ecs->url()}thanh-vien/?act=order_detail&order_id=' . $order_id . "\n");
    exit;
}
...
elseif ($action == 'save_order_address')
{
    ...
    {
        ecs_header('Location: {$ecs->url()}thanh-vien/?act=order_detail&order_id=' .$address['order_id']. "\n");
        exit;
    }
    else
    {
...
elseif ($action == 'affiliate')
{
    ...
    if(empty($goodsid))
    {
        ...
        $url_format = $ecs->url()."thanh-vien/?act=affiliate&page=";
...
elseif ($action =='email_list')
{
    ...
    if ($job == 'add')
    {
        if (empty($ck))
        {
            ...
            $url = $ecs->url() . "thanh-vien?act=email_list&job=add_check&hash=$hash&email=$email";
            send_mail('', $email, $_LANG['check_mail'], sprintf($_LANG['check_mail_content'], $email, $GLOBALS['_CFG']['shop_name'], $url, $url, $GLOBALS['_CFG']['shop_name'], local_date('Y-m-d')), 1);
        }
        elseif ($ck['stat'] == 1)
        {
    ...
    elseif ($job == 'del')
    {
        ...
        elseif ($ck['stat'] == 1)
        {
            ...
            $db->query($sql);
            $info = $_LANG['email_check'];
            $url = $ecs->url() . "thanh-vien?act=email_list&job=del_check&hash=$hash&email=$email";
            send_mail('', $email, $_LANG['check_mail'], sprintf($_LANG['check_mail_content'], $email, $GLOBALS['_CFG']['shop_name'], $url, $url, $GLOBALS['_CFG']['shop_name'], local_date('Y-m-d')), 1);
        }
        else
        {
...
else if ($action == 'track_packages')
{
    ...
    $pager  = get_pager($ecs->url().$ecs->url().'thanh-vien', array('act' => $action), $record_count, $page);
    $smarty->assign('pager',  $pager);
    $smarty->assign('orders', $orders);
    $_template = 'user_transaction'.$_device.'.dwt';
    $smarty->display($_template);
...
elseif ($action == 'act_transform_points')
{
    ...
    {
        show_message($_LANG['invalid_points'], $_LANG['transform_points'], $ecs->url().'thanh-vien/?act=transform_points');
    }
    ...
    {
        show_message($_LANG['overflow_points'], $_LANG['transform_points'], $ecs->url().'thanh-vien/?act=transform_points' );
    }
    ...
    switch ($rule_key)
    {
        case TO_P :
            ...
            show_message(sprintf($_LANG['to_pay_points'],  $num, $points_name[$bbs_key]['title'], $result_points), $_LANG['transform_points'], $ecs->url().'thanh-vien/?act=transform_points');

        case TO_R :
            $result_points = floor($num * $to / $from);
            $user->set_points($row['user_name'], array($bbs_key=>0 - $num)); //调整论坛积分
            log_account_change($row['user_id'], 0, 0, $result_points, 0, $_LANG['transform_points'], ACT_OTHER);
            show_message(sprintf($_LANG['to_rank_points'], $num, $points_name[$bbs_key]['title'], $result_points), $_LANG['transform_points'], $ecs->url().'thanh-vien/?act=transform_points');
        ...
        case FROM_P :
            ...
            show_message(sprintf($_LANG['from_pay_points'], $num, $result_points,  $points_name[$bbs_key]['title']), $_LANG['transform_points'], $ecs->url().'thanh-vien/?act=transform_points');

        case FROM_R :
            $result_points = floor($num * $to / $from);
            log_account_change($row['user_id'], 0, 0, 0-$num, 0, $_LANG['transform_points'], ACT_OTHER); //调整商城积分
            $user->set_points($row['user_name'], array($bbs_key=>$result_points)); //调整论坛积分
            show_message(sprintf($_LANG['from_rank_points'], $num, $result_points, $points_name[$bbs_key]['title']), $_LANG['transform_points'], $ecs->url().'thanh-vien/?act=transform_points');
    }
}
elseif ($action == 'act_transform_ucenter_points')
{
    ...
    if ($ratio == 0)
    {
        show_message($_LANG['exchange_deny'], $_LANG['transform_points'], $ecs->url().'thanh-vien/?act=transform_points');
    }
    ...
    if ($result === true)
    {
        ...
        $db->query($sql);
        show_message(sprintf($_LANG['exchange_success'], $exchange_amount, $_LANG['exchange_points'][$fromcredits], $netamount, $credit['title']), $_LANG['transform_points'], $ecs->url().'thanh-vien/?act=transform_points');
    }
    else
    {
        show_message($_LANG['exchange_error_1'], $_LANG['transform_points'], $ecs->url().'thanh-vien/?act=transform_points');
    }
}
/* 清除商品浏览历史 */
elseif ($action == 'clear_history')
{
    setcookie('ECS[history]',   '', 1, $cookie_path, $cookie_domain, $cookie_secure, $cookie_http_only);
}
?>