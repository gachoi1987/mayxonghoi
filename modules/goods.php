...
$is_real  = $db->getOne("SELECT is_real FROM " . $ecs->table('goods') ." WHERE goods_id=".$goods_id);

$cache_id = $goods_id.$is_real . '-' . $_SESSION['user_rank'].'-'.$_CFG['lang'].'-'.$_device;
$cache_id = sprintf('%X', crc32($cache_id));
/**
 * Layout thay đổi theo biến is_real
 * goods:mặc định | goods2:Hàng cũ | goods3:Preorder | goods4:Mới ra mắt
 */
if(isset($tragop) && $tragop == 0){
    $_template =  $is_real > 2 ? 'goods'.$is_real.$_device.'.dwt': 'goods'.$_device.'.dwt';
}else{
...
if (!$smarty->is_cached($_template, $cache_id))
{
    ...
    else
    {
        ...
        $smarty->assign('province_list', $region);
        ...
        $catlist = get_parent_cats_new($goods['cat_id']);
        ...
        /* current position */
        if(isset($tragop) && $tragop == 1){
            $titletg = 'Mua trả góp - ';
            $position = assign_ur_here(0, 'Mua trả góp '.$goods['goods_name'], '');
            $smarty->assign('tragop_list', get_tragop());
        ...
        $smarty->assign('page_title',          $titletg.htmlspecialchars($goods['title']));
        $smarty->assign('description',          $titletg.htmlspecialchars($goods['description']));
        ...
        $smarty->assign('parent_cat_name',        $catlist['cat_name']);
        $smarty->assign('shoplist',        get_agency());
        //获取tag
...
function get_tragop($id='')
{
   $sql = "SELECT * FROM ".$GLOBALS['ecs']->table('tragop')." ORDER BY tragop_id DESC";
