...
{
    show_message($_LANG['ws_user_rank'], $_LANG['ws_return_home'], "../\n");
}
/*------------------------------------------------------ */
//-- act 操作项的初始化
/*------------------------------------------------------ */
...
if ($_REQUEST['act'] == 'list')
{
    ...
    if ($count > 0)
    {
        ...
        $param['act'] = 'list';
        $pager = get_pager($ecs->url().'ban-si', array_reverse ($param, TRUE), $count, $page, $size);
    ...
    /* 模板赋值 */
    assign_template();
    $position = assign_ur_here('', '', 'wholesale');
...
elseif ($_REQUEST['act'] == 'add_to_cart')
{
    ...
    /* 刷新页面 */
    ecs_header("Location: {$ecs->url()}ban-si\n");
    exit;
}
/*------------------------------------------------------ */
//-- 从购物车删除
...
elseif ($_REQUEST['act'] == 'submit_order')
{
    ...
    /* 提示 */
    show_message(sprintf($_LANG['ws_order_submitted'], $order['order_sn']), $_LANG['ws_return_home'], "../\n");
}
/**
 * 取得某页的批发商品
