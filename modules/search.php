...
$_template = "search".$_device.".dwt";

$_REQUEST['act'] = !empty($_REQUEST['act']) ? trim($_REQUEST['act']) : '';

    $_REQUEST['keywords']   =  htmlspecialchars($_REQUEST['keywords']);
    ...
    // $sqlcat = "SELECT cat_id, cat_name, cat_url_seo, sort_order FROM " .$ecs->table('category').
    //           " WHERE parent_id = 0 AND is_show = 1 ORDER BY sort_order, cat_id";
    // $rescat = $db->getAll($sqlcat);
    // var_dump($rescat);

    /* 查询商品 */
    $sql = "SELECT g.goods_id, g.cat_id, g.goods_name, g.online_price, g.market_price, g.seller_note, g.is_on_sale , g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ".
