...
if ($action == 'act_add_message')
{
    ...
    $message = array(
        ...
        'msg_title'   => isset($_POST['msg_title']) ? htmlspecialchars(trim($_POST['msg_title']))     : '',
        'msg_content' => isset($_POST['msg_content']) ? htmlspecialchars(trim($_POST['msg_content'])) : '',
        'order_id'    => 0,
        'msg_area'    => 1,
        'upload'      => array()
     );
    /** XSS attack */
    $message['msg_content'] = strip_tags($message['msg_content']);
    $message['user_name'] = strip_tags($message['user_name']);
    $message['user_tel']  = strip_tags($message['user_tel']);
    $message['user_email'] = strip_tags($message['user_email']);
    $message['msg_title'] = strip_tags($message['msg_title']);
    ...
    if (add_message($message))
    {
        ...
        show_message($msg_info, $_LANG['message_list_lnk'], '/lien-he');
    }
    else
    {
        $err->show($_LANG['message_list_lnk'], '/lien-he');
    }
}
if ($action == 'default')
{
    $_template = 'message_board'.$_device.'.dwt';
    assign_template();
    $position = assign_ur_here(0, $_LANG['message_board'], 'message');
    ...
    // $pager = get_pager('/lien-he', array(), $record_count, $page, $pagesize);
